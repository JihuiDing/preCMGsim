...
## Overview
the preCMGsim repo processes and converts a Petrel geomodel into CMG compatible formats, and set up CMG dat files that fully define fluid flow and geomechanical coupled simulations. With inputs of realizations of reservoir properties (e.g., porosity and permeability) from files generated by a Petrel Uncertainty & Optimization workflow, this repo automatically sets up CMG dat files that will be run by CMG simulators. Therefore, the workflow implemented in this repo is well suited for uncertainty quantification. Below are high-level steps.
